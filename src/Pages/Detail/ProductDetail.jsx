...
const ProductDetail = () => {
    ...
    if (setProceed) {
      try {
        const { data } = await axios.post(
          "http://206.189.135.103:5000/api/addToCart",
  ...
  const addToWhishList = async (product) => {
    if (setProceed) {
      try {
        const { data } = await axios.post(
          `http://206.189.135.103:5000/api/addToWishlist`,
          { _id: product._id },
          {
            headers: {
              Authorization: authToken,
  ...
  const getSimilarProducts = async () => {
    const { data } = await axios.get(
      `http://206.189.135.103:5000/api/getProduct/shoe`
    );
    setSimilarProduct(data.result);
  };
  let data = [];
  if (cat === "shoe") {
